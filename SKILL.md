...
2. **判断信息类型 → 按类型选源**（titles-source-map §0.5.2 决策表）：官方名录类（院士/Fellow）→ 官方名单；机构任职类（职称/院长）→ 单位官网师资页；活动角色类（编委/会议主席）→ 期刊/会议官网；履历类（海外经历）→ 个人 CV/主页。**源不确定是常态，按类型换源、不写死名单**
3. **构造 query**（§0.5.3）："中文名+单位+头衔词" / 英文名+title / site: 限定官方域名；查不到先换变体再下结论
4. **交叉验证**：至少两个独立来源一致（官方库 ↔ 高校官网/喜报 ↔ 论文致谢 NSFC 资助号），结论标注 确认/疑似/存疑/无法验证
5. **时效**：名单逐年动态；2024 年起杰青不再公布完整名单；会议主席/编委以当年官网为准并标注查询日期
